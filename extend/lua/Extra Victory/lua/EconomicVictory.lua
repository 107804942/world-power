...
function CalculateEconomicVictoryProgress(iPlayer)
	if bVictoryEnabled then
		...
		if (Game.GetElapsedGameTurns() == 1) or (Game.GetElapsedGameTurns() % 15 == 0) then
	...
	for pResource in GameInfo.Resources() do
		...
		if (iResourceCount > 0) then
			if (pResource.ResourceClassType == "RESOURCECLASS_LUXURY") then
			 ...
			 if  pResource.Type ~= "RESOURCE_PORCELAIN"  --´ÉÆ÷
			 and pResource.Type ~= "RESOURCE_FOIEGRAS"   --¶ì¸Î
			 and pResource.Type ~= "RESOURCE_GLASS"      --²£Á§
			 and pResource.Type ~= "RESOURCE_JEWELRY"    --Öé±¦
			 and pResource.Type ~= "RESOURCE_PEPPER"     --ºú½·
			 then
