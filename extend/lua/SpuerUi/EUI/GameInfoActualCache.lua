--==========================================================
-- ÏÄÆ¤ÂÞµÄ°ô×Ó
-- rog edit: added SV Icons for Resources without textures
-- TO DO: UnitPromotions
--==========================================================
...
local g_MissingTextures = {
	RESOURCE_PAPER		= "SV_Paper.dds",
	RESOURCE_FUSIONCORE	= "SV_FusionCore.dds"
}
...
--print( "Accessing data base table", tableName )
			local QueryAll = DB.CreateQuery( "SELECT * from "..tableName )
			local QueryID = keys.ID and DB.CreateQuery( "SELECT * from "..tableName.." WHERE ID = ?" )
			local QueryType = keys.Type and DB.CreateQuery( "SELECT * from "..tableName.." WHERE Type = ?" )
...
--print( "Creating index for key", key )
				...
				for i = 1, #set do -- iterate over set row array
					...
					if v ~= nil then -- does row have a defined value for this akey ?
						...
						table.insert( subset, r ) -- add row to subset row array
					end
				end
