...
function GetUnitsString(plot)
	...
	for i = 0, numUnits - 1 do
		...
		if (unit ~= nil and not unit:IsInvisible(iActiveTeam, bIsDebug)) then
			...
			strUnitText = strUnitText .. ", " .. hp .. "[ICON_HP]";
			...
			for unitPromotion in GameInfo.UnitPromotions() do
				if (unit:IsHasPromotion(unitPromotion.ID) and unitPromotion.ShowInUnitPanel ~= 0 ) then
					...
					if (promotion == nil or promotion[2] < tonumber(rank:sub(2))) then
					    rankedPromotions[base] = {unitPromotion.IconString2, tonumber(rank:sub(2))};
					end
					...
					local promotion = propertyPromotions[group];
					
					if (promotion == nil) then
					    propertyPromotions[group] = {unitPromotion.IconString2, 0};
					end
					propertyPromotions[group][2] = propertyPromotions[group][2]+1;
				    else
	 ...
	 -------------------------------- 新增----------------------------------------
				...
				elseif unitPromotion.IconAtlas ~= "PROMOTION_ATLAS"
					...
					local promotion = groupedPromotions[group];
					
					if (promotion == nil) then
					    groupedPromotions[group] = {unitPromotion.IconString2, 0};
					end
					
					groupedPromotions[group][2] = groupedPromotions[group][2]+1;
				    end
				end
			end
			...
			for _,promotion in pairs(propertyPromotions) do
				table.insert(promotionList, promotion);
			end
			for _,promotion in pairs(rankedPromotions) do
				promotion[2] = 1;
				table.insert(promotionList, promotion);
			end
			for _,promotion in pairs(groupedPromotions) do
				table.insert(promotionList, promotion);
			end
			
			for _,promotion in pairs(promotionList) do
				if bIsFirstPromotion then
					strUnitText = strUnitText .. ", ";
					bIsFirstPromotion = false;
				end
				
				strUnitText = strUnitText .. promotion[1];
