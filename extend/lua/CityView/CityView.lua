-- cityview8
-- Author: 11585
-- DateCreated: 2022/8/23 20:38:13
--------------------------------------------------------------
...
local g_DoctorIM   = InstanceManager:new( "SpecialistInstance", "SpecialistButton", Controls.BoxODoctors );
...
local MusicianTexture = "citizenIntellectual.dds"; --修改
local artistTexture = "citizenArtist.dds"; --修改
local WriterTexture = "citizenDoctor3.dds"; --修改
...
local doctorTexture = "citizenDoctor.dds"; --新增
--local unemployedTexture = "citizenUnemployed.dds"; 
local unemployedTexture = "citizenPriest.dds";  
...
function AddBuildingButton( pCity, building )
	...
	if (pCity:IsHasBuilding(buildingID))  then --新增PLAGUE MOD support for compatibility
		...
		if pCity:GetNumBuilding(buildingID) > 1 then
			if buildingID == GameInfoTypes["BUILDING_ADV_ARMORY"]
			or buildingID == GameInfoTypes["BUILDING_ADV_HARBOR"]
			then
				strBuildingName = strBuildingName .. " (" .. pCity:GetNumBuilding(buildingID)-1 .. ")";
			else
		...
		-- Special Tool Tip for Military Headquarters
		if     buildingID == GameInfoTypes["BUILDING_ADV_ARMORY"] then
			if pCity:GetNumBuilding(GameInfoTypes["BUILDING_ADV_ARMORY"]) == 1 then
				strToolTip = strToolTip .. "[NEWLINE][NEWLINE]" .. Locale.ConvertTextKey("TXT_KEY_BUILDING_ADV_ARMORY_NULL");
			else
				strToolTip = strToolTip .. "[NEWLINE][NEWLINE]" .. Locale.ConvertTextKey("TXT_KEY_BUILDING_ADV_ARMORY_FULL", pCity:GetNumBuilding(GameInfoTypes["BUILDING_ADV_ARMORY"])-1);
			end
		elseif buildingID == GameInfoTypes["BUILDING_ADV_HARBOR"] then
			if pCity:GetNumBuilding(GameInfoTypes["BUILDING_ADV_HARBOR"]) == 1 then
				strToolTip = strToolTip .. "[NEWLINE][NEWLINE]" .. Locale.ConvertTextKey("TXT_KEY_BUILDING_ADV_HARBOR_NULL");
			else
				strToolTip = strToolTip .. "[NEWLINE][NEWLINE]" .. Locale.ConvertTextKey("TXT_KEY_BUILDING_ADV_HARBOR_FULL", pCity:GetNumBuilding(GameInfoTypes["BUILDING_ADV_HARBOR"])-1);
			end
		end
	...
	-- Culture & Yield
				...
				BToolTipString = BToolTipString .. Locale.ConvertTextKey("TXT_KEY_CITYVIEW_SPECIALIST_TOOLTIP", GameInfo.Specialists[iSpecialistID].Description);
					...
					------------------------ 新增------------------------
                if building.SpecialistType == "SPECIALIST_DOCTOR" then	
					controlTable.SlotTexture:SetTexture(doctorTexture);
					controlTable.BuildingSpecialistSlot:SetTexture(doctorTexture);
					------------------------ end------------------------
				
				elseif iSpecialistID == 1 then	-- SPECIALIST_WRITER
		...
		if iSpecialistID then
			...
			for i = 1, specialistsAllowedNum do
				if iSpecialistID ~= 0 then		-- SPECIALIST_CITIZEN
					...
					elseif iSpecialistID == 6 then	-- SPECIALIST_ENGINEER
						...
						------------------------------------------新增------------------------------------------
					elseif iSpecialistID == GameInfo.Specialists{Type="SPECIALIST_DOCTOR"}().ID   then	-- 新增doctor
						controlTable = g_DoctorIM:GetInstance();
						controlTable.FilledSpecialistSlot:SetTexture(doctorTexture);-- end
						------------------------------------------end------------------------------------------
					end
			...
			if specialistAdded > 0 then
				...
				elseif iSpecialistID == 6 then
					...
					------------------------------------------新增------------------------------------------
				elseif iSpecialistID == GameInfo.Specialists{Type="SPECIALIST_DOCTOR"}().ID then --新增
					Controls.BoxODoctors:SetSize( frameSize );--end
					------------------------------------------end------------------------------------------
...
-------------------------------------------------------- end--------------------------------------------------------
		...
		local numDoctorsInThisCity    = pCity:GetSpecialistCount( GameInfo.Specialists.SPECIALIST_DOCTOR.ID );
		...
		for building in GameInfo.Buildings() do
			...
			then
				...
				elseif building.SpecialistType == "SPECIALIST_DOCTOR" then
					iNumDoctors  = iNumDoctors  + pCity:GetNumSpecialistsAllowedByBuilding(buildingID);
				else
		...
		local specialistSlotsSum = iNumWriters + iNumArtists + iNumMusicians + iNumScientists + iNumMerchants + iNumEngineers+iNumDoctors;
		local specialistSum = numWritersInThisCity + numArtistsInThisCity + numMusiciansInThisCity + numScientistsInThisCity + numMerchantsInThisCity + numEngineersInThisCity+ numDoctorsInThisCity;
		...
		if iNumWriters > 0 and specialistHeadingOpen then
			...
			Controls.WriterName:SetText( Locale.ConvertTextKey("TXT_KEY_SPECIALISTSANDGP_WRITER_TITLE") .. " [ICON_GREAT_WRITER]" );
		...
		if iNumArtists > 0 and specialistHeadingOpen then
			...
			Controls.ArtistName:SetText( Locale.ConvertTextKey("TXT_KEY_SPECIALISTSANDGP_ARTIST_HEADING3_TITLE") .. " [ICON_GREAT_ARTIST]" );
		...
		if iNumMusicians > 0 and specialistHeadingOpen then
			...
			Controls.MusicianName:SetText( Locale.ConvertTextKey("TXT_KEY_SPECIALISTSANDGP_MUSICIAN_TITLE") .. " [ICON_GREAT_MUSICIAN]" );
		...
		if iNumScientists > 0 and specialistHeadingOpen then
			...
			Controls.ScientistName:SetText( Locale.ConvertTextKey("TXT_KEY_SPECIALISTSANDGP_SCIENTIST_HEADING3_TITLE") .. " [ICON_GREAT_SCIENTIST]" );
		...
		if iNumMerchants > 0 and specialistHeadingOpen then
			...
			if pPlayer:MayNotAnnex() then
				Controls.MerchantName:SetText( Locale.ConvertTextKey("TXT_KEY_SPECIALISTSANDGP_MERCHANT_HEADING3_TITLE") .. " [ICON_GREAT_MERCHANT_VENICE]" );
			else
				Controls.MerchantName:SetText( Locale.ConvertTextKey("TXT_KEY_SPECIALISTSANDGP_MERCHANT_HEADING3_TITLE") .. " [ICON_GREAT_MERCHANT]" );
			end
		...
		if iNumEngineers > 0 and specialistHeadingOpen then
			...
			Controls.EngineerName:SetText( Locale.ConvertTextKey("TXT_KEY_SPECIALISTSANDGP_ENGINEER_HEADING3_TITLE") .. "[ICON_GREAT_ENGINEER]" );
		...
		if iNumDoctors > 0 and specialistHeadingOpen then
			...
			Controls.BoxODoctors:SetVoid1( 7 );
			Controls.BoxODoctors:SetVoid2(-1 );
			Controls.BoxODoctors:RegisterCallback( Mouse.eLClick, AddSpecialist );
			Controls.BoxODoctors:RegisterCallback( Mouse.eRClick, RemoveSpecialist );
			Controls.DoctorName:SetText( Locale.ConvertTextKey("TXT_KEY_SPECIALISTSANDGP_DOCTOR_HEADING3_TITLE") .. " [ICON_GREAT_DOCTOR]" );
		...
		--local numDoctorInThisCity     = pCity:GetSpecialistCount( GameInfo.Specialists.SPECIALIST_DOCTOR.ID );
...
--		UpdateSpecialists(pCity);
		...
		if  iCityHealthIndex < 0 then   ---健康度不足
			...
			Controls.CityHealthLabel:SetText( Locale.ConvertTextKey("TXT_KEY_CITYVIEW_PERTURN_TEXT_NEGATIVE2", iCityHealthIndex, HealthCounter, plagueThreshold) );
		else
			Controls.CityHealthLabel:SetText( Locale.ConvertTextKey("TXT_KEY_CITYVIEW_PERTURN_TEXT2", iCityHealthIndex, HealthCounter, plagueThreshold) )
		end
...
----------------------------------------end----------------------------------------
		...
		Controls.TourismPerTurnLabel:SetText( Locale.ConvertTextKey("TXT_KEY_CITYVIEW_PERTURN_TOURISM_TEXT", iTourismPerTurn) );

		----------------------------------------goldenage----------------------------------------
		local yieldGoldenAgeID   = GameInfo.Yields["YIELD_GOLDEN_AGE_POINTS"].ID
		local igoldenagePerTurn = pCity:GetBaseYieldRate(yieldGoldenAgeID) + pCity:GetLocalHappiness()
		Controls.GoldenagePerTurnLabel:SetText( Locale.ConvertTextKey("TXT_KEY_CITYVIEW_PERTURN_TEXT", igoldenagePerTurn) );
	----------------------------------------end----------------------------------------
		...
		local XPSea = pCity:GetDomainFreeExperience(domainSeaID)+pCity:GetDomainFreeExperienceFromGreatWorks(domainSeaID)+pCity:GetDomainFreeExperienceFromGreatWorksGlobal(domainSeaID) +pPlayer:GetDomainFreeExperience(domainSeaID)

        XPAir = XPAir + XP+ XP2+ XP3
		XPLand = XPLand + XP+ XP2+ XP3
		XPSea = XPSea + XP+ XP2+ XP3


		Controls.LandXpLabel:SetText( Locale.ConvertTextKey("TXT_KEY_CITYVIEW_XP_TEXT",XPLand) );
	...
	----------------------------------------end----------------------------------------
		...
		if (UI.IsCityScreenViewingMode()) then
			...
			Controls.BoxODoctors:SetDisabled( true );--新增
			...
			Controls.BoxODoctors:SetDisabled( false );--新增
...
function DoUpdateProductionInfo( bNoProduction )
	...
	strToolTip = strToolTip .. strOutput;
		...
		-------------------------------新增---------------------------------
	local szHelpText = "";
		--if (pCity~=nil and pCity:IsProductionBuilding()) then
			local cityProductionBuilding = pCity:GetProductionBuilding();
			if cityProductionBuilding ~= -1 then
			szHelpText = GetHelpTextForBuilding( cityProductionBuilding, true, true, true, pCity );
			--end
	  end
    strToolTip = strToolTip .. szHelpText;
	---------------------------------end-------------------------------
...
function DoUpdateUpperLeftTooltips()
	...
	local pCity = UI.GetHeadSelectedCity();
	local iCityOwner = pCity:GetOwner() --新增
	local player = Players[iCityOwner]  --新增
	...
	--新增goldenage 
	local strGoldenageToolTip = GetGoldenageTooltip(player, pCity);
	Controls.GoldenageBox:SetToolTipString(strGoldenageToolTip);
	--end addition
...
Controls.BTNCityAuto:RegisterCheckHandler(OnCityAutomation)
	...
	function GetGoldenageTooltip(player, city)
	    local yieldGoldenAgeID   = GameInfo.Yields["YIELD_GOLDEN_AGE_POINTS"].ID
		local GAPTT = "";
		--Local Happiness
		...
		if GAPLocalHappiness > 0 then
			...
			GAPTT = GAPTT .. Locale.ConvertTextKey("TXT_KEY_CITYVIEW_ROG_GAP_TT_FROM_LOCAL_HAPPINESS", GAPLocalHappiness)
		end
		--Buildings
		...
		if GAPBuilding ~= 0 then
			...
			GAPTT = GAPTT .. Locale.ConvertTextKey("TXT_KEY_CITYVIEW_ROG_GAP_TT_FROM_BUILDINGS", GAPBuilding)
		end
		--Religion
		...
		if GAPReligion ~= 0 then
			...
			GAPTT = GAPTT .. Locale.ConvertTextKey("TXT_KEY_CITYVIEW_ROG_GAP_TT_FROM_RELIGION", GAPReligion)
		end
		--Specialists
		...
		if GAPSpecialist ~= 0 then
			...
			GAPTT = GAPTT .. Locale.ConvertTextKey("TXT_KEY_CITYVIEW_ROG_GAP_TT_FROM_SPECIALISTS", GAPSpecialist)
		end
		--Terrain
		...
		if GAPTerrain ~= 0 then
			...
			GAPTT = GAPTT .. Locale.ConvertTextKey("TXT_KEY_CITYVIEW_ROG_GAP_TT_FROM_TERRAIN", GAPTerrain)
		end
		--Units
		...
		--Modifier
		local GAPModifier = city:GetYieldRateModifier(yieldGoldenAgeID)
		 + player:GetYieldRateModifier(yieldGoldenAgeID) 
		 -- + city:GetModFromWLTKD(yieldGoldenAgeID)
		...
		if GAPModifier ~= 0 then
			...
			GAPTT = GAPTT .. Locale.ConvertTextKey("TXT_KEY_CITYVIEW_ROG_GAP_TT_FROM_MODIFIERS", GAPModifier)
		end
		--local GAPTotal = player:GetCityGAP(city)
		local GAPTotal =  city:GetYieldRate(yieldGoldenAgeID) + city:GetLocalHappiness()
		return Locale.ConvertTextKey("TXT_KEY_ROG_CITYVIEW_GAP_TEXT_TT", GAPTT, GAPTotal)
	end
