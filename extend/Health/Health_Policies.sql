...
SELECT 'POLICY_STATECRAFT',								    'BUILDINGCLASS_ACADEMY',	'YIELD_HEALTH',		2	UNION ALL
SELECT 'POLICY_STATECRAFT',								    'BUILDINGCLASS_LIBRARY',	'YIELD_HEALTH',		2	UNION ALL
SELECT 'POLICY_STATECRAFT',							        'BUILDINGCLASS_UNIVERSITY',	'YIELD_HEALTH',		2	UNION ALL
SELECT 'POLICY_STATECRAFT',							        'BUILDINGCLASS_PUBLIC_SCHOOL',	'YIELD_HEALTH',	2	UNION ALL
...
UPDATE LocalizedText
	SET Text = Text||'[NEWLINE][NEWLINE]每个已采纳的政策提升'||(SELECT Value FROM Temp WHERE Tag = Temp.Key)||'%'||'[ICON_HEALTH]健康度。'
...
UPDATE LocalizedText
	SET Text = Text||'[NEWLINE][ICON_BULLET]+'||(SELECT Value FROM Temp WHERE Tag = Temp.Key)||'%'||'[ICON_HEALTH]健康度。'
	WHERE Tag IN (SELECT Key FROM Temp);
