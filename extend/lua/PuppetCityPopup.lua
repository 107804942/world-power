...
PopupLayouts[ButtonPopupTypes.BUTTONPOPUP_CITY_CAPTURED] = function(popupInfo)
	...
	if (bRaze) then
		local OnRazeClicked = function()
			...
			----------------------------------------’®“©ø∆ºº√Î≥«------------------------------------------------
		if Teams[activePlayer:GetTeam()]:IsHasTech(GameInfo.Technologies["TECH_DYNAMITE"].ID) then
		local plot = newCity:Plot()
	    local hexpos = ToHexFromGrid(Vector2(plot:GetX(), plot:GetY()));
	    --local cityID = newCity:GetID()
		  local iPlayer = Game.GetActivePlayer()
		  newCity:Kill();
		  Events.SerialEventCityDestroyed(hexpos, iPlayer, cityID, -1)
