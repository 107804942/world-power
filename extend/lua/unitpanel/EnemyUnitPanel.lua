	...
	if (pMyUnit ~= nil) then
		...
		--------------------------------------------------新增抵扣的最终伤害--------------------------------------------------
			...
			if  pCity:GetChangeDamageValue() ~= 0 then
				...
				controlTable = g_TheirCombatDataIM:GetInstance();
				controlTable.Text:LocalizeAndSetText("TXT_KEY_EUPANEL_CHANGE_DAMAGEVALUE_SUPPORT",ChangeDamageValue);
		   ...
		   -----------------------------------------------新增抵扣的最终伤害------------
			...
			then
				controlTable = g_MyCombatDataIM:GetInstance();		
				controlTable.Text:LocalizeAndSetText("TXT_KEY_EUPANEL_CHANGE_DAMAGEVALUE_SUPPORT",UnitChangeDamageValue);
				controlTable.Value:SetText("");
			end
			...
			if (iModifier ~= 0 and  pMyPlayer:GetNumOriginalCapital()>1 ) then
			 ...
			 iModifier = pMyUnit:GetNumOriginalCapitalAttackMod()* inum
				controlTable = g_MyCombatDataIM:GetInstance();		
				controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_PLAYER_ORIGINAL_CAPITAL_BONUS");
				controlTable.Value:SetText( GetFormattedText(strText, iModifier, true, true) );
			end
			...
			------新增与首都同大陆加成
			  ...
			  if (iModifier ~= 0 and  pArea == pCapitalArea) then
				controlTable = g_MyCombatDataIM:GetInstance();		
				controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_PLAYER_ON_SAME_CAPITAL_LAND_BONUS");
				controlTable.Value:SetText( GetFormattedText(strText, iModifier, true, true) );
				end
			end
			...
			------新增与首都不同大陆加成
			  ...
			  if (iModifier ~= 0 and  pArea ~= pCapitalArea) then
				controlTable = g_MyCombatDataIM:GetInstance();		
				controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_PLAYER_ON_DIFFERENT_CAPITAL_LAND_BONUS");
				controlTable.Value:SetText( GetFormattedText(strText, iModifier, true, true) );
				end
			end
			...
			if (iModifier ~= 0 and pMyUnit:MovesLeft()>0) then
				...
				controlTable = g_MyCombatDataIM:GetInstance();		
				controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_MOVES_LEFT_BONUS");
				controlTable.Value:SetText( GetFormattedText(strText, iModifier, true, true) );
			end
		...
		------新增已用移动力加成
			...
			if (iModifier ~= 0 and  (pMyUnit:MaxMoves() > pMyUnit:MovesLeft())) then
				...
				controlTable = g_MyCombatDataIM:GetInstance();		
				controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_MOVES_USED_BONUS");
				controlTable.Value:SetText( GetFormattedText(strText, iModifier, true, true) );
			end
			...
			if (iModifier ~= 0 and Hitbonus~=0 ) then
				...
				controlTable = g_MyCombatDataIM:GetInstance();		
				controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_LEFT_HITPOINT_BONUS");
				controlTable.Value:SetText( GetFormattedText(strText, iModifier,true, true) );
			end
			...
			if (iModifier ~= 0 and  pMyPlayer:GetNumWorldWonders()>0) then
				...
				controlTable = g_MyCombatDataIM:GetInstance();		
				controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_PLAYER_WORLDWONDER_BONUS");
				controlTable.Value:SetText( GetFormattedText(strText, iModifier, true, true) );
			end
			...
			if (iModifier ~= 0 and  pMyPlayer:GetNumSpies()>0) then
				...
				controlTable = g_MyCombatDataIM:GetInstance();		
				controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_PLAYER_SPY_BONUS");
				controlTable.Value:SetText( GetFormattedText(strText, iModifier, true, true) );
			end
...
function UpdateCombatOddsUnitVsUnit(pMyUnit, pTheirUnit)
	...
	--------------------------SP Bug Fix END ------------Air unit attack infantry unit wrong damage---------
			...
			local myMaxUnitHitPoints =  pTheirUnit:GetMaxHitPoints(); 
			local theirMaxUnitHitPoints = pMyUnit:GetMaxHitPoints();
			...
			DoUpdateHealthBars(theirMaxUnitHitPoints, myMaxUnitHitPoints , pMyUnit:GetDamage(), pTheirUnit:GetDamage(), iMyDamageInflicted, iTheirDamageInflicted)
			...
			if(pMyUnit:IsNoResourcePunishment() == true) then
				controlTable = g_MyCombatDataIM:GetInstance();
				controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_NO_RS_PSH" );
				controlTable.Value:SetText("");
			end
...
----------------------------------------------------------------晋升对晋升的显示效果----------------------------------------------------------------
   ...
   ----------------------------------------------------------------晋升对晋升的显示效果end----------------------------------------------------------------
	...
	if (pToPlot:IsOpenGround()) then
			...
			-- force damage --
			-------------------------
			--local ForcedDamageValue=pTheirUnit:GetForcedDamageValue()
			...
			-- Movement Immunity ----
			-------------------------
			movementTheir = pTheirUnit:GetZOCStatus();
			...
			if (pTheirUnit:IsCombatUnit()) then
				...
				if (iModifier ~= 0) then
				if (not bRanged) then
				--if UI.GetInterfaceMode()==InterfaceModeTypes.INTERFACEMODE_ATTACK or UI.GetInterfaceMode() == InterfaceModeTypes.INTERFACEMODE_MOVE_TO then
					controlTable = g_TheirCombatDataIM:GetInstance();
	...
	----------------------------------------------------------------晋升对晋升的显示效果----------------------------------------------------------------
		-- Promotions
		local AnotherPromotions = {};
		local AnotherPromotions3 = {};
			for row in GameInfo.UnitPromotions_PromotionModifiers() do
			local PromotionInfo = GameInfo.UnitPromotions[row.OtherPromotionType] 
			local PromotionInfoMain = GameInfo.UnitPromotions[row.PromotionType]
			if PromotionInfo~=nil then 
		    if pTheirUnit:IsHasPromotion(PromotionInfoMain.ID) then
			if pMyUnit:IsHasPromotion(PromotionInfo.ID) then
			...
			elseif (row.Defense < 0) then
				row.Defense = "[COLOR_NEGATIVE_TEXT]" .. row.Defense .. "[ENDCOLOR]";
			            ...
			            if row.Modifier~=0 then
						table.insert( AnotherPromotions, PromotionInfo.IconString2 .." :  " .. Locale.ConvertTextKey( row.Modifier.. "%" ) );
						end

						if row.Defense~=0 then
						table.insert( AnotherPromotions3, PromotionInfo.IconString2 .." :  ".. Locale.ConvertTextKey( row.Defense.. "%" ) );
						end
		...
		local AnotherpromotionText = {};
		local AnotherpromotionText3 = {};
		for _,promotion in pairs(AnotherPromotions) do
			table.insert(AnotherpromotionText, promotion);
		end
		for _,promotion in pairs(AnotherPromotions3) do
			table.insert(AnotherpromotionText3, promotion);
		end
			...
			iModifier=iModifier+row.Modifier 
	      	...
	      	if (iModifier ~= 0) then
				controlTable = g_TheirCombatDataIM:GetInstance();			
				controlTable.Text:LocalizeAndSetText( Locale.ConvertTextKey("TXT_KEY_EUPANEL_BONUS_VS_OTHER_PROMOTION").."".."[NEWLINE]" ..table.concat(AnotherpromotionText, "[NEWLINE]" ) );
				controlTable.Value:SetText( GetFormattedText(strText, iModifier,  false, true) );
            end


				-- UnitPromotions_PromotionDefense
		    local iModifier =0
			for row in GameInfo.UnitPromotions_PromotionModifiers() do
			local PromotionInfo = GameInfo.UnitPromotions[row.OtherPromotionType] 
			local PromotionInfoMain = GameInfo.UnitPromotions[row.PromotionType] 
		    ...
		    if pTheirUnit:IsHasPromotion(PromotionInfoMain.ID) then
			if pMyUnit:IsHasPromotion(PromotionInfo.ID) then
			iModifier=iModifier+row.Defense
			        end
			     end
			  end
			end
	      	if (iModifier ~= 0) then
				controlTable = g_TheirCombatDataIM:GetInstance();
				controlTable.Text:LocalizeAndSetText( Locale.ConvertTextKey("TXT_KEY_EUPANEL_BONUS_VS_OTHER_PROMOTION_DEFENSE").."".."[NEWLINE]" ..table.concat( AnotherpromotionText3, "[NEWLINE]" ) )
				controlTable.Value:SetText( GetFormattedText(strText, iModifier, false, true) );
            end
  ----------------------------------------------------------------晋升对晋升的显示效果end----------------------------------------------------------------
	...
	--				strString.append(GetLocalizedText("TXT_KEY_COMBAT_PLOT_EXTRA_STRENGTH", iModifier));
			...
			if (iModifier ~= 0) then
				...
				if (pToPlot:IsOpenGround()) then
					...
					--if (iModifier ~= 0 and (not bRanged)) then  ----对远程无效
					  if (iModifier ~= 0 ) then
						controlTable = g_TheirCombatDataIM:GetInstance();
...
function OnMouseOverHex( hexX, hexY )
	...
	--Events.ClearHexHighlightStyle("ValidFireTargetBorder") --rog
