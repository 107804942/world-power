...
local g_KnowledgePipeManager = InstanceManager:new( "ConnectorPipe", "ConnectorImage", Controls.KnowledgePanel );
local g_IndustryPipeManager = InstanceManager:new( "ConnectorPipe", "ConnectorImage", Controls.IndustryPanel );
local g_GrandeurPipeManager = InstanceManager:new( "ConnectorPipe", "ConnectorImage", Controls.GrandeurPanel );
...
local g_KnowledgeInstanceManager = InstanceManager:new( "PolicyButton", "PolicyIcon", Controls.KnowledgePanel );
local g_IndustryInstanceManager = InstanceManager:new( "PolicyButton", "PolicyIcon", Controls.IndustryPanel );
local g_GrandeurInstanceManager = InstanceManager:new( "PolicyButton", "PolicyIcon", Controls.GrandeurPanel );
...
function UpdateDisplay()
	local bDoneKnowledge = false
	local bDoneGrandeur = false
	local bDoneIndustry = false
    local player = Players[Game.GetActivePlayer()];
...
--	if numUnlockedBranches > 0 then
		...
		while policyBranchInfo ~= nil and not policyBranchInfo.PurchaseByLevel and i < 12 do
			--print(GameInfo.PolicyBranchTypes[i])
			--print(i)
			...
			if numString == "11" then 
				--print(":)") 
				thisLock = Controls.Lockgrandeur 
			end
			...
			if not player:IsPolicyBranchUnlocked( i ) then
				...
				elseif (not player:CanUnlockPolicyBranch(i)) then
					--print(ImageMaskName .. "IsPolicyBranchLocked")
				...
				-- Can adopt this branch right now
					...
					--print(ImageMaskName .. "IsPolicyBranchAdoptable")
			...
			elseif (player:IsPolicyBranchBlocked(i)) then
				--print(ImageMaskName .. "IsPolicyBranchBlocked")
			...
			-- Branch is unlocked already
				...
				--print(ImageMaskName .. "Branch is unlocked already")
			...
			--[[ if not bDoneKnowledge and (not policyBranchInfo or policyBranchInfo.PurchaseByLevel) then
			--	print("!")
				policyBranchInfo = GameInfo.PolicyBranchTypes[GameInfoTypes["POLICY_BRANCH_KNOWLEDGE"]]
			--[[ 	i = 9
				bDoneKnowledge = true
			elseif not bDoneGrandeur and bDoneKnowledge then
			--	print("!!")
				policyBranchInfo = GameInfo.PolicyBranchTypes[GameInfoTypes["POLICY_BRANCH_GRANDEUR"]]
			--[[ 	i = 11
				bDoneGrandeur = true
			elseif not bDoneIndustry and bDoneKnowledge and bDoneGrandeur then
			--	print("!!!")
				policyBranchInfo = GameInfo.PolicyBranchTypes[GameInfoTypes["POLICY_BRANCH_INDUSTRY"]]
			--[[ 	i = 10
				bDoneIndustry = true
			else
			--	print(":|")
	...
	while policyInfo ~= nil do
		...
		if policyInfo.PolicyBranchType then
			iBranch = policyInfo.PolicyBranchType;
		end
...
function GetPipe(branchType)
	...
	elseif branchType == "POLICY_BRANCH_KNOWLEDGE" then
		controlTable = g_KnowledgePipeManager:GetInstance();
	elseif branchType == "POLICY_BRANCH_GRANDEUR" then
		controlTable = g_GrandeurPipeManager:GetInstance();
	elseif branchType == "POLICY_BRANCH_INDUSTRY" then
...
function Init()
	...
	while policyInfo ~= nil do
		...
		if (iBranch ~= nil) then
			...
			elseif iBranch == "POLICY_BRANCH_KNOWLEDGE" then
				controlTable = g_KnowledgeInstanceManager:GetInstance();
			elseif iBranch == "POLICY_BRANCH_GRANDEUR" then
				controlTable = g_GrandeurInstanceManager:GetInstance();
			elseif iBranch == "POLICY_BRANCH_INDUSTRY" then
...
Controls.CloseButton2:RegisterCallback(Mouse.eLClick, function() 
	Players[Game.GetActivePlayer()]:SetNumFreePolicies(0);
	Players[Game.GetActivePlayer()]:SetNumFreeTenets(0);
	Controls.FreeTenetsLabel:SetHide( true );
	Controls.FreePoliciesLabel:SetHide( true );
	print("clear free policy")
end);
