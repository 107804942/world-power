...
SELECT 'POLICY_FREE_RELIGION',								'BUILDINGCLASS_CLOISTER','YIELD_HEALTH',	1	UNION ALL
...
UPDATE Language_zh_CN
	SET Text = Text||'[NEWLINE][NEWLINE]每个已采纳的政策提升全局[ICON_HEALTH]健康度'||(SELECT Value FROM Temp WHERE Tag = Temp.Key)||'%。'
...
UPDATE Language_zh_CN
	SET Text = Text||'[NEWLINE]+'||(SELECT Value FROM Temp WHERE Tag = Temp.Key)||'%'||'全局[ICON_HEALTH]健康度。'
	WHERE Tag IN (SELECT Key FROM Temp);
...
UPDATE Language_zh_CN
	SET Text = Text||'[NEWLINE]每个'||(SELECT Building FROM Temp WHERE Tag = Temp.Key)||'+'||(SELECT Value FROM Temp WHERE Tag = Temp.Key)||'%'||'全局[ICON_HEALTH]健康度(上限10%)。'
	WHERE Tag IN (SELECT Key FROM Temp);
