...
-------------------------------------------------
	...
	Controls.HealthString:RegisterCallback( eLClick,  function() LuaEvents.PLAGUE_CityHealthListPopup() end )
	end
...
local function CreateIcon( index, texture, ToolTipHandler, OnLClick, OnRClick, ID )
	...
	if ID == GameInfo.Resources.RESOURCE_MANPOWER.ID then
		instance.Image:SetTexture( "sv_manpower.dds" )
		instance.Image:SetTextureSizeVal( 160, 160 )
		instance.Image:NormalizeTexture()

	-- nanomat icon
	elseif ID ==GameInfo.Resources.RESOURCE_TROOPS.ID   then
		instance.Image:SetTexture( "manpower.dds" )
		instance.Image:SetTextureSizeVal( 160, 160 )
		instance.Image:NormalizeTexture()	
	
    --electricity icon
	elseif ID == GameInfo.Resources.RESOURCE_ELECTRICITY.ID then
		instance.Image:SetTexture( "sv_electricity.dds" )
		instance.Image:SetTextureSizeVal( 160, 160 )
		instance.Image:NormalizeTexture()
	
	 ---consumer goods icon
	elseif ID == GameInfo.Resources.RESOURCE_CONSUMER.ID  then
...
---新增健康度
for k in ("SciencePerTurn TechIcon GoldPerTurn  GpIcon HappinessString GoldenAgeString CultureString FaithString FaithIcon TourismString InternationalTradeRoutes HealthString "):gmatch("(%S*)%s*") do
