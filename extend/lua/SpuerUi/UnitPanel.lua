-- Lua Script4
-- Author: 11585
-- DateCreated: 2021/12/7 18:58:10
--------------------------------------------------------------
...
function UpdateUnitPortrait(  unit )
	...
	------------------------------------------------------新增------------------------------------------------------------------------------------------
	if  unit:GetUnitType() ~= GameInfoTypes["UNIT_SPACESHIP"] 
		and unit:GetUnitType() ~= GameInfoTypes["UNIT_MECH"]
		and unit:GetUnitType() ~= GameInfoTypes["UNIT_GOLEM"]
		and unit:GetUnitType() ~= GameInfoTypes["UNIT_GHOST"]
		and unit:GetUnitType() ~= GameInfoTypes["UNIT_SIEGE04H"]
		and unit:GetUnitType() ~= GameInfoTypes["UNIT_LEVDESTROYER"]
		and unit:GetUnitType() ~= GameInfoTypes["UNIT_NAVALCARRIER03P"]
		and unit:GetUnitType() ~= GameInfoTypes["UNIT_ANGEL"]
		and unit:GetUnitType() ~= GameInfoTypes["UNIT_NAVALCARRIER03S"]
		and unit:GetUnitType() ~= GameInfoTypes["UNIT_NAVAL_MONSTER"]
		and unit:GetUnitType() ~= GameInfoTypes["UNIT_ALIEN_SIEGE_WORM"]
		and unit:GetUnitType() ~= GameInfoTypes["UNIT_PARTICLE_CANNON"] then
	

	--if not unit:IsHasPromotion(GameInfoTypes["PROMOTION_ELITE_UNIT"]) then --新增
	if  unit:GetMaxHitPoints() > 1000  then
		name = "[ICON_PROMOTION_GROUP_EXTRA][ICON_PROMOTION_GROUP_EXTRA][ICON_PROMOTION_GROUP_EXTRA][ICON_PROMOTION_ELITE_EXTRA]" .. name;
	elseif unit:GetMaxHitPoints() >  900 then
		name = "[ICON_PROMOTION_GROUP_EXTRA][ICON_PROMOTION_GROUP_EXTRA][ICON_PROMOTION_ELITE_EXTRA][ICON_PROMOTION_ELITE_EXTRA]" .. name;
	elseif unit:GetMaxHitPoints() >  800 then
		name = "[ICON_PROMOTION_GROUP_EXTRA][ICON_PROMOTION_GROUP_EXTRA][ICON_PROMOTION_GROUP_EXTRA]" .. name;
	elseif unit:GetMaxHitPoints() >  700  then
		name = "[ICON_PROMOTION_GROUP_EXTRA][ICON_PROMOTION_GROUP_EXTRA][ICON_PROMOTION_ELITE_EXTRA]" .. name;
	elseif unit:GetMaxHitPoints() >  600  then
		name = "[ICON_PROMOTION_GROUP_EXTRA][ICON_PROMOTION_ELITE_EXTRA][ICON_PROMOTION_ELITE_EXTRA]" .. name;
	elseif unit:GetMaxHitPoints() >  500  then
		name = "[ICON_PROMOTION_GROUP_EXTRA][ICON_PROMOTION_GROUP_EXTRA]" .. name;
	elseif unit:GetMaxHitPoints() >  400  then
		name = "[ICON_PROMOTION_GROUP_EXTRA][ICON_PROMOTION_ELITE_EXTRA]" .. name;
	elseif unit:GetMaxHitPoints() >  300  then
		name = "[ICON_PROMOTION_ELITE_EXTRA][ICON_PROMOTION_ELITE_EXTRA]" .. name;
	elseif unit:GetMaxHitPoints() >  200  then
		name = "[ICON_PROMOTION_GROUP_EXTRA]" .. name;
	elseif unit:GetMaxHitPoints() >  100  then
		name = "[ICON_PROMOTION_ELITE_EXTRA]" .. name;
	    end

end


	if     unit:GetUnitType() == GameInfoTypes["UNIT_SPACESHIP"] then --新增
	name = "[ICON_PROMOTION_ELITE_EXTRA][ICON_PROMOTION_ELITE_EXTRA]" .. name.. "[ICON_PROMOTION_ELITE_EXTRA][ICON_PROMOTION_ELITE_EXTRA]";
	end
	----------------------------------------------------------------------------------------------------------------------------------end
	if     unit:IsHasPromotion(GameInfoTypes["PROMOTION_CAN_RANGE_ATTACK"]) then --新增
	name = name .. "[ICON_RANGE_STRENGTH]";
	end
...
function UpdateCityPortrait(city)
    ...
    Controls.XPFrame:SetHide( true );
    Controls.SecondaryButton:SetHide(true);
	Controls.UnitStatBox:SetHide(true);  ----新增
    Controls.UnitStatBox2:SetHide(true);  ----新增
...
function UpdateUnitPromotionsCondensed(unit)
    ...
    for unitPromotion in GameInfo.UnitPromotions() do
        if (unit:IsHasPromotion(unitPromotion.ID)  and unitPromotion.ShowInUnitPanel ~= 0 ) then
            ...
            elseif unitPromotion.OrderPriority == 11 then --优先定义
                
                -- Assumes promotion icons are in an 8x8 grid
                local group = 100 - unitPromotion.PortraitIndex;
                local promotions = propertyPromotions[group];
                
                if (promotions == nil) then
                    promotions = {};
                    propertyPromotions[group] = promotions;
                end
                
                promotions[type] = unitPromotion;
            else
                local atlas = 0;	       -- "PROMOTION_ATLAS" or "ABILITY_ATLAS"
                if     unitPromotion.IconAtlas == "EXPANSION_PROMOTION_ATLAS" then
                    atlas = 1;
                elseif unitPromotion.IconAtlas == "EXPANSION2_PROMOTION_ATLAS" then
                    atlas = 2;
                elseif unitPromotion.IconAtlas == "KRIS_SWORDSMAN_PROMOTION_ATLAS" then
                    atlas = 3;
                elseif unitPromotion.IconAtlas == "SP_PROMOTION_ATLAS" then
                    atlas = 4;
    -------------------------------- 新增----------------------------------------
                 elseif unitPromotion.IconAtlas == "MainRobotPromo_Atlas" then
                    atlas = 5;
                 elseif unitPromotion.IconAtlas == "robotPromo_Atlas" then
                    atlas = 6;
                 elseif unitPromotion.IconAtlas == "promoVP_atlas_00" then
                    atlas = 7;
				elseif unitPromotion.IconAtlas == "promoVP_atlas_01" then
                    atlas = 8;
				elseif unitPromotion.IconAtlas == "promoVP_atlas_02" then
                    atlas = 9;
				elseif unitPromotion.IconAtlas == "promoVP_atlas_03" then
                    atlas = 10;
				elseif unitPromotion.IconAtlas == "promoMUC_atlas_00" then
                    atlas = 11;
				elseif unitPromotion.IconAtlas == "promoMUC_atlas_01" then
                    atlas = 12;
                elseif unitPromotion.IconAtlas == "VALERY_ATLAS" then
                    atlas = 13;
				 elseif unitPromotion.IconAtlas == "MORE_NEW_PROMOS_ATLAS" then
                    atlas = 14;	
					
				elseif unitPromotion.IconAtlas == "SPY_PROMOTION_ATLAS" then
					    atlas = 15;	

				elseif unitPromotion.IconAtlas == "CIV_COLOR_ATLAS_FW_PROMOTIONS" then
					    atlas = 16;				
-------------------------------- 新增----------------------------------------
                elseif unitPromotion.IconAtlas ~= "PROMOTION_ATLAS"
                and    unitPromotion.IconAtlas ~= "ABILITY_ATLAS"
                then
                    atlas = 17;
                end
                
                -- Assumes promotion icons are in an 8x8 grid
                local group = atlas * 100 + unitPromotion.PortraitIndex;
    ...
    addGroupedPromotions(propertyPromotions);
...
function addGroupedPromotions(groupedPromotions)
    ...
    for _,group in pairs(groupKeys) do
        local promotions = groupedPromotions[group];
...
function UpdateUnitStats(unit)
    ...
    IconHookup( civPortraitIndex, 128, civInfo.IconAtlas, Controls.BackgroundCivSymbol );
	 ...
	 Controls.UnitStatBox:SetHide( bIsBNW == true and unit:IsTrade() )
	 
	 -----------------------------------------------------------------------
	 local movesLeft = unit:MovesLeft()
	 if movesLeft >= unit:MaxMoves() then
		Controls.MovementPip:SetTextureOffsetVal( 0, 0 )-- cyan (green)
	elseif movesLeft > 0 then
		if unit:IsCombatUnit() and unit:IsOutOfAttacks() then
			Controls.MovementPip:SetTextureOffsetVal( 0, 96 )-- orange (gray)
		else
			Controls.MovementPip:SetTextureOffsetVal( 0, 32 )-- green (yellow)
		end
	else
		Controls.MovementPip:SetTextureOffsetVal( 0, 64 )-- red
	end
	 -----------------------------------------------------------------------
    ...
    if unit:GetDomainType() == DomainTypes.DOMAIN_AIR then
		...
		Controls.UnitStatMovement:SetText(iRange .. "[ICON_MOVES]");
		...
		--Controls.UnitStatNameMovement:SetToolTipString(szMoveStr);
		Controls.UnitStatMovement:SetToolTipString(Locale.ConvertTextKey( "TXT_KEY_UPANEL_UNIT_MAY_MOVE", moves_left ));
    end
    ...
    elseif (unit:GetSpreadsLeft() > 0) then
        ...
        Controls.UnitStatStrength:SetToolTipString(strengthTT);
        --Controls.UnitStatNameStrength:SetToolTipString(strengthTT);
    elseif bIsBNW == true and unit:GetTourismBlastStrength() > 0 then
   ...
   local iMAXHP = 0;
		local Currenthp = 0;
		if unit~=nil then
		iMAXHP =unit:GetMaxHitPoints() ;
		Currenthp = unit:GetCurrHitPoints()              
		--local szMoveStr = iRange .. " [ICON_MOVES]";
		Controls.UnitStatBox2:SetHide(bIsBNW == true and unit:IsTrade());
		Controls.UnitStatHp:SetText(Currenthp .. "/" ..iMAXHP.." ".."[ICON_SILVER_FIST]".." ");
		Controls.UnitStatHp:SetToolTipString(Locale.ConvertTextKey( "TXT_KEY_UPANEL_HP_TT"));

		----------------------------------------------------------------	
        if (unit:CargoSpace() > 0) and bIsBNW == true then
        --Controls.UnitRangedAttackBox:SetHide(false);
        --local rangeStrengthStr = Locale.ConvertTextKey( "TXT_KEY_UPANEL_CARGO_CAPACITY" );
        --Controls.UnitStatNameRangedAttack:SetText(rangeStrengthStr);
        --Controls.UnitStatCargo:SetText(" <> "..unit:CargoSpace() .. "     ");  
		Controls.UnitStatCargo:SetText("[ICON_CARGOSPACE]"..unit:CargoSpace().."     ");  
		  
        local rangeStrengthTT = Locale.ConvertTextKey( "TXT_KEY_UPANEL_CARGO_CAPACITY_TT", unit:GetName());
        Controls.UnitStatCargo:SetToolTipString(rangeStrengthTT);
        --Controls.UnitStatNameRangedAttack:SetToolTipString(rangeStrengthTT);   
       elseif unit:CargoSpace() == 0  then
	  ---Controls.UnitRangedAttackBox:SetHide(true);
	  Controls.UnitStatCargo:SetText()
         end      
		end
	Controls.UnitStats2:CalculateSize()
	Controls.UnitStats2:ReprocessAnchoring()   
...
function TipHandler( control )
	...
	if (bBuild) then
		...
		------------------------------------------------新增------------------------------------------------
		   ...
		   --HEALTH 新增
			--for row in GameInfo.Improvement_Yields("YieldType = 'YIELD_HEALTH'") do
			--if row.ImprovementType==GameInfo.Builds[iBuildID].ImprovementType then
				--iYieldHealth = row.Yield
				--if (iYieldHealth <0) then
				    --strBuildYieldString = strBuildYieldString .."[NEWLINE]"..Locale.ConvertTextKey("TXT_KEY_BUILD_HEALTH_STRING_3", iYieldHealth);
				--else
					--strBuildYieldString = strBuildYieldString .."[NEWLINE]".. Locale.ConvertTextKey("TXT_KEY_BUILD_HEALTH_STRING2", iYieldHealth);
					--end
		        --end
		  -- end
		   ------------------------------------------------end------------------------------------------------
		...
		do
			...
			if (iYieldChange ~= 0) then
				...
				elseif (not bFirstEntry and bFirstYield) then
					strBuildYieldString = strBuildYieldString 
				end
				...
				elseif (iYield == YieldTypes.YIELD_GOLDEN_AGE_POINTS) then
					strBuildYieldString = strBuildYieldString .. Locale.ConvertTextKey("TXT_KEY_BUILD_GOLDEN_AGE_POINTS_STRING", iYieldChange);
				 --大军点数
				elseif (iYield == YieldTypes.YIELD_GREAT_GENERAL_POINTS) then
					strBuildYieldString = strBuildYieldString .. Locale.ConvertTextKey("TXT_KEY_BUILD_GREAT_GENERAL_POINTS_STRING", iYieldChange);
	
				--海军上将
				elseif (iYield == YieldTypes.YIELD_GREAT_ADMIRAL_POINTS) then
					strBuildYieldString = strBuildYieldString .. Locale.ConvertTextKey("TXT_KEY_BUILD_GREAT_ADMIRAL_POINTS_STRING", iYieldChange);
				--健康度新增
				elseif (iYield == YieldTypes.YIELD_HEALTH) then
					strBuildYieldString = strBuildYieldString .. Locale.ConvertTextKey("TXT_KEY_BUILD_HEALTH_STRING", iYieldChange);
				--疾病新增
				elseif (iYield == YieldTypes.YIELD_DISEASE) then
					strBuildYieldString = strBuildYieldString .. Locale.ConvertTextKey("TXT_KEY_BUILD_DISEASE_STRING", iYieldChange);
				 --犯罪率
				elseif (iYield == YieldTypes.YIELD_CRIME) then
					strBuildYieldString = strBuildYieldString .. Locale.ConvertTextKey("TXT_KEY_BUILD_CRIME_STRING", iYieldChange);
				--忠诚度
				elseif (iYield == YieldTypes.YIELD_LOYALTY) then
					strBuildYieldString = strBuildYieldString .. Locale.ConvertTextKey("TXT_KEY_BUILD_LOYALTY_STRING", iYieldChange);
				--集权
				elseif (iYield == YieldTypes.YIELD_SOVEREIGNTY) then
					strBuildYieldString = strBuildYieldString .. Locale.ConvertTextKey("TXT_KEY_BUILD_SOVEREIGNTY_STRING", iYieldChange);
				end
