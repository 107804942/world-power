...
local bWPIsActive = false
for _,v in ipairs(Modding.GetActivatedMods()) do
	if (v.ID == "41450919-c52c-406f-8752-5ea34be32b2d") then
		bWPIsActive = true
	end
end
...
function CanPlaceVolcanoAt(feature, plot)
	...
	if (plot:GetFeatureType() ~= NO_FEATURE) then
		return false;
	end
	...
	-- Check for being too close to another volcano
	local uniqueRange = feature.VolcanoRange;
	local plotX = plot:GetX();
	local plotY = plot:GetY();
	for dx = -uniqueRange, uniqueRange - 1, 1 do
		for dy = -uniqueRange, uniqueRange - 1, 1 do
			local otherPlot = Map.PlotXYWithRangeCheck(plotX, plotY, dx, dy, uniqueRange);
			if(otherPlot and otherPlot:GetFeatureType() == featureID) then
				return false;
			end
		end
	end 
