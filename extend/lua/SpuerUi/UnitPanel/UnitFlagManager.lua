    ...
    if (pPlot:IsCity()) then
        ...
        for i = 0, pPlot:GetNumUnits() - 1 do
            ...
            if pAirUnit:GetDomainType() == DomainTypes.DOMAIN_AIR and not pAirUnit:IsInvisible(Game.GetActiveTeam()) then
                ...
                controlTable.Button:SetToolTipType("EUI_UnitTooltip");
                controlTable.Button:SetToolTipCallback(UnitFlagToolTipCall);
            ...
            if (pCarrierUnit:HasCargo() and not pCarrierUnit:IsInvisible(Game.GetActiveTeam())) then
                ...
                for j = 0, pPlot:GetNumUnits() - 1 do
                    ...
                    then
                        ...
                        cargoCoTable.Button:SetToolTipType("EUI_UnitTooltip");
                        cargoCoTable.Button:SetToolTipCallback(UnitFlagToolTipCall);
...
local function AddPromotionIcon(flag, promoID, iconPositionID)
    ...
    if not IconHookup( promo.PortraitIndex, 45, promo.IconAtlas, button ) then   -- 16 → 45
        print("No 45x45 icon for ".. promo.Type .. " failing back to the yellow triangle")
        IconHookup( 59, 45, "PROMOTION_ATLAS", button )                          -- 16 → 45
    end
