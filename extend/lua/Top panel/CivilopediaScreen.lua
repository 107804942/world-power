...
local projectsToIgnore = {
    PROJECT_BAZOOKA_TRAINING = true,  --不显示
...
CivilopediaCategory[CategoryPromotions].PopulateList = function()
	...
	for thisPromotion in GameInfo.UnitPromotions() do
		if thisPromotion.PediaType == "PEDIA_MELEE" 
		 and thisPromotion.ShowInUnitPanel ~= 0 and thisPromotion.ShowInPedia ~= 0   then
			-- add an article to the list (localized name, unit tag, etc.)
			local article = {};
	...
	local tableid = 1;
	for thisPromotion in GameInfo.UnitPromotions() do
		if thisPromotion.PediaType == "PEDIA_RANGED" 
		 and thisPromotion.ShowInUnitPanel ~= 0 and thisPromotion.ShowInPedia ~= 0   then
			-- add an article to the list (localized name, unit tag, etc.)
			local article = {};
			local name = Locale.ConvertTextKey( thisPromotion.PediaEntry )
	...
	local tableid = 1;
	for thisPromotion in GameInfo.UnitPromotions() do
		if thisPromotion.PediaType == "PEDIA_NAVAL" 
		 and thisPromotion.ShowInUnitPanel ~= 0 and thisPromotion.ShowInPedia ~= 0   then
			-- add an article to the list (localized name, unit tag, etc.)
			local article = {};
			local name = Locale.ConvertTextKey( thisPromotion.PediaEntry )
	...
	local tableid = 1;
	for thisPromotion in GameInfo.UnitPromotions() do
		if thisPromotion.PediaType == "PEDIA_HEAL" 
		 and thisPromotion.ShowInUnitPanel ~= 0 and thisPromotion.ShowInPedia ~= 0   then
			-- add an article to the list (localized name, unit tag, etc.)
			local article = {};
			local name = Locale.ConvertTextKey( thisPromotion.PediaEntry )
	...
	local tableid = 1;
	for thisPromotion in GameInfo.UnitPromotions() do
		if thisPromotion.PediaType == "PEDIA_SCOUTING" 
		 and thisPromotion.ShowInUnitPanel ~= 0 and thisPromotion.ShowInPedia ~= 0   then
			-- add an article to the list (localized name, unit tag, etc.)
			local article = {};
			local name = Locale.ConvertTextKey( thisPromotion.PediaEntry )
	...
	local tableid = 1;
	for thisPromotion in GameInfo.UnitPromotions() do
		if thisPromotion.PediaType == "PEDIA_AIR" 
		 and thisPromotion.ShowInUnitPanel ~= 0 and thisPromotion.ShowInPedia ~= 0   then
			-- add an article to the list (localized name, unit tag, etc.)
			local article = {};
			local name = Locale.ConvertTextKey( thisPromotion.PediaEntry )
	...
	local tableid = 1;
	for thisPromotion in GameInfo.UnitPromotions() do
		if thisPromotion.PediaType == "PEDIA_SHARED"
		 and thisPromotion.ShowInUnitPanel ~= 0 and thisPromotion.ShowInPedia ~= 0   then
			-- add an article to the list (localized name, unit tag, etc.)
			local article = {};
			local name = Locale.ConvertTextKey( thisPromotion.PediaEntry )
	...
	local tableid = 1;
	for thisPromotion in GameInfo.UnitPromotions() do
		if thisPromotion.PediaType == "PEDIA_ATTRIBUTES" 
		 and thisPromotion.ShowInUnitPanel ~= 0 and thisPromotion.ShowInPedia ~= 0   then
			-- add an article to the list (localized name, unit tag, etc.)
			local article = {};
			local name = Locale.ConvertTextKey( thisPromotion.PediaEntry )
...
CivilopediaCategory[CategoryPeople].PopulateList = function()
	...
	--------------------新增---------------
		if unit.PrereqTech == nil and unit.Special ~= nil 
		and  unit.Type ~= "UNIT_MODERN_GREAT_DOCTOR"  then
--------------------end---------------
			local article = {};
			local name = Locale.ConvertTextKey( unit.Description )
			article.entryName = name;
...
CivilopediaCategory[CategoryUnits].SelectArticle = function( unitID, shouldAddToList )
	...
	if unitID ~= -1 then
		...
		for row in GameInfo.Unit_FreePromotions( condition ) do
			...
			if promotion and promotion.ShowInUnitPanel ~= 0 and promotion.ShowInPedia ~= 0  then
...
CivilopediaCategory[CategoryPromotions].SelectArticle = function( promotionID, shouldAddToList )
	...
	if promotionID ~= -1 and  GameInfo.UnitPromotions[promotionID].ShowInUnitPanel ~= 0 and GameInfo.UnitPromotions[promotionID].ShowInPedia ~= 0  then
		...
		if thisPromotion.PromotionPrereqOr10 then
			local thisReq = GameInfo.UnitPromotions[thisPromotion.PromotionPrereqOr10];
			if thisReq then
				local thisRequiredPromotionInstance = g_RequiredPromotionsManager:GetInstance();
				if thisRequiredPromotionInstance then
					local textureOffset, textureSheet = IconLookup( thisReq.PortraitIndex, buttonSize, thisReq.IconAtlas );				
		...
		if thisPromotion.PromotionPrereqOr11 then
			local thisReq = GameInfo.UnitPromotions[thisPromotion.PromotionPrereqOr11];
			if thisReq then
				local thisRequiredPromotionInstance = g_RequiredPromotionsManager:GetInstance();
				if thisRequiredPromotionInstance then
					local textureOffset, textureSheet = IconLookup( thisReq.PortraitIndex, buttonSize, thisReq.IconAtlas );				
		...
		if thisPromotion.PromotionPrereqOr12 then
			local thisReq = GameInfo.UnitPromotions[thisPromotion.PromotionPrereqOr12];
			if thisReq then
				local thisRequiredPromotionInstance = g_RequiredPromotionsManager:GetInstance();
				if thisRequiredPromotionInstance then
					local textureOffset, textureSheet = IconLookup( thisReq.PortraitIndex, buttonSize, thisReq.IconAtlas );				
		...
		if thisPromotion.PromotionPrereqOr13 then
			local thisReq = GameInfo.UnitPromotions[thisPromotion.PromotionPrereqOr13];
			if thisReq then
				local thisRequiredPromotionInstance = g_RequiredPromotionsManager:GetInstance();
				if thisRequiredPromotionInstance then
					local textureOffset, textureSheet = IconLookup( thisReq.PortraitIndex, buttonSize, thisReq.IconAtlas );				
		...
		for row in GameInfo.UnitPromotions{PromotionPrereqOr9 = thisPromotion.Type} do
			...
			for row in GameInfo.UnitPromotions{PromotionPrereqOr10 = thisPromotion.Type} do
			local unlocked = GameInfo.UnitPromotions[row.Type];
			if unlocked then
				local thisUnlockedInstance = g_UnlockedPromotionsManager:GetInstance();
				if thisUnlockedInstance then
		...
		for row in GameInfo.UnitPromotions{PromotionPrereqOr11 = thisPromotion.Type} do
			local unlocked = GameInfo.UnitPromotions[row.Type];
			if unlocked then
				local thisUnlockedInstance = g_UnlockedPromotionsManager:GetInstance();
				if thisUnlockedInstance then
		...
		for row in GameInfo.UnitPromotions{PromotionPrereqOr12 = thisPromotion.Type} do
			local unlocked = GameInfo.UnitPromotions[row.Type];
			if unlocked then
				local thisUnlockedInstance = g_UnlockedPromotionsManager:GetInstance();
				if thisUnlockedInstance then
		...
		for row in GameInfo.UnitPromotions{PromotionPrereqOr13 = thisPromotion.Type} do
			local unlocked = GameInfo.UnitPromotions[row.Type];
			if unlocked then
				local thisUnlockedInstance = g_UnlockedPromotionsManager:GetInstance();
				if thisUnlockedInstance then
...
CivilopediaCategory[CategoryPeople].SelectArticle =  function( rawPeopleID, shouldAddToList )
	...
	if rawPeopleID < 1000 then
		...
		--if rawPeopleID ~= -1 and rawPeopleID~=GameInfo.Units["UNIT_MODERN_GREAT_DOCTOR"].ID then
		---end
