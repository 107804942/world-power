...
function RefreshCityBanner(cityBanner, iActiveTeam, iActivePlayer)
	...
	--------------------------------------------SP City Show City Focus END--------------------------
		...
		if  city:HasPlague()  then  
			...
			--controls.PlagueIcon:SetToolTipString(Locale.ConvertTextKey( "TXT_KEY_PM_CITY_PLAGUED", iTurns));
			controls.PlagueIcon:LocalizeAndSetToolTip("TXT_KEY_CITYBANNER_HEALTH_TEXT_TT_PLAGUE", plague.IconString, plague.Description, city:GetPlagueTurns())
		else
		...
		if cityOwner:IsHuman() and controls.CityIcon_Lv1 ~= nil then
		...
		if (controls.CityProductionName) then
			...
			if (controls.CityBannerLeftBackground) then
				if city:IsProductionProcess() then
					...
					local tip = GetHelpTextForProcess(cityProductionProcess, false)
					--controls.CityBannerLeftBackground:SetToolTipString(tooltipString)

					local thisProcessInfo = GameInfo.Processes[cityProductionProcess];
				...
				elseif city:IsProductionUnit() then
					...
					local portraitOffset, portraitAtlas = UI.GetUnitPortraitIcon(cityProductionUnit, city:GetOwner());		
					local tip = GetHelpTextForUnit2(cityProductionUnit)
				...
				elseif city:IsProductionBuilding() then
					...
					local tip = GetHelpTextForBuilding(cityProductionBuilding, true, true, true, city)
					--controls.CityBannerLeftBackground:SetToolTipString(tooltipString)

					local thisBuildingInfo = GameInfo.Buildings[cityProductionBuilding];
				...
				elseif city:IsProductionProject() then
					...
					local tip = GetHelpTextForProject(cityProductionProject, true, city)
					--controls.CityBannerLeftBackground:SetToolTipString(tooltipString)

					local thisProjectInfo = GameInfo.Projects[cityProductionProject];
